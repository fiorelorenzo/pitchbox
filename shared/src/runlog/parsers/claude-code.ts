import type { ParsedEvent, CliEnvelope } from '../types.js';
...
/**
 * Parses one line of Claude Code `stream-json` output into normalized events.
 * The caller provides `seqStart`; returned events have seq = seqStart, seqStart+1, ...
 */
export function parseClaudeCodeLine(line: string, seqStart = 0): ParsedEvent[] {
  if (!line.trim()) return [];
