...
import { parseCodexLine } from '../runlog/parsers/codex.js';
...
/**
 * Codex CLI runner. Spawns `codex exec --json <prompt>` and pipes stdout
 * through `parseCodexLine`.
 *
 * ASSUMED INVOCATION: `codex exec --json <prompt>` (subject to drift across
 * versions of OpenAI's `@openai/codex` CLI). Model is passed via
 * `--model <model>` when configured; default assumed model is `gpt-5-codex`.
 *
 * The playbook is read from disk and prepended to the prompt so the agent
 * has the same context as the claude-code runner.
 */
    ...
    const result: Promise<AgentRunResult> = (async () => {
      ...
      const exitCode = await new Promise<number>((resolve, reject) => {
        ...
        const emitLine = (line: string) => {
          ...
          console.log(`[codex ${opts.slug}] ${line.slice(0, 200)}`);
          opts.onRawLine?.(line);

          const events = parseCodexLine(line, seq);
