...
| `ASSIST_TOKEN_BUDGET`      | 60_000 input tokens | Accumulated across steps. Past it, no further tool result is admitted into the context and the agent is told to answer. **Measured for #574 and left unchanged**: the worst realistic tool loop tops out around 7,000 tokens on any one turn - never close to this ceiling (`shared/src/assist/budget.ts` carries the full measured table).                                                                                                                |
| `ASSIST_COST_CEILING_USD`  | 0.5                 | **New for #574.** A per-suggestion USD ceiling, enforced the same way (past it, no more tools, answer with what you have) - distinct from the org's own monthly Gateway budget and the instance ceiling below, both of which stay hard aborts on a different plane. Sized with roughly 2x headroom over the heaviest estimated real case (premium model, every tool, one image); `shared/src/assist/budget.ts` carries the full measured table and method. |
...
- #574 measured the loop's real cost, confirmed `ASSIST_TOKEN_BUDGET` was
  already right, and added `ASSIST_COST_CEILING_USD` - the issue that may
  move any figure in the table above. Anyone else changing one edits this
