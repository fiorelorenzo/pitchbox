...
CI enforcement (LI-11): a test over `extension/src/` that fails on a `fetch`/`XMLHttpRequest` whose URL mentions linkedin, on a `document.cookie`, `chrome.cookies`, `localStorage`, or `sessionStorage` read under a LinkedIn content script, and on a synthetic `.click()`/`dispatchEvent` targeting a LinkedIn control. A rule that only lives in a document is a rule that a future agent breaks while every test passes. The scan set deliberately excludes `chrome.storage.local`/`.sync`: it is the extension's own sandboxed namespace, isolated per-extension by the browser, and cannot reach anything LinkedIn owns - the pairing lookup both LinkedIn panel scripts already make through `pickPairing()` -> `getSettings()` relies on exactly this being permitted (LOR-283).
