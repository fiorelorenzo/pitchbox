...
- `reply-poller.ts` - drives the `ReplyReader` interface (`shared/src/platforms/base-reply-reader.ts`), but only for platforms with a real (non-Null) reader registered (`reply-readers.ts`). Mastodon has a real reader (mentions via its API), so the poller polls it; Reddit is only wired with a `NullReplyReader` and is skipped, its reply detection going through the Chrome extension's `inbox-sync`/`chat-sync` pollers instead, which POST to `/api/extension/dm-sync`.
...
- With `PITCHBOX_AUTH=on`, the daemon authenticates its own `POST /api/run` dispatch (`scheduler.ts` and `keyword-watcher.ts`) with `PITCHBOX_INTERNAL_TOKEN`, sent as `Authorization: Bearer <token>`. The web hook accepts that header only for `/api/run`, only on a request with no session, and only when the token matches - see [`auth.md`](./auth.md). Without it configured, that dispatch keeps getting a 401 exactly as it always has, and no scheduled or keyword-triggered campaign can run (#378). Both processes still need to read the same value, which in practice means the same `.env` on the same host, or the same secret wired into each process's environment.
