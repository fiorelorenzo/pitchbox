 ...
 *  - POST /api/settings/extension-pairing: role-gated only, no tenant guard -
 *    the cleanest place to exercise the "no locals.org" no-op case, since a
 *    tenant-guarded route would 404 before ever reaching requireRole. It
 *    creates a pairing scoped to the caller's own org (resolveOrgId), so a
 *    per-org admin genuinely should pass this gate - unlike PUT
 *    /api/settings/webhooks, which used to sit here: that route writes a
 *    single instance-wide app_config row, so it is instance-admin-gated as of
 *    #137, not requireRole-gated; see instance-admin-gating.test.ts. (Same
 *    reasoning rules out /api/settings/default-runner, /quota,
 *    /runner-config, and the /api/settings/retention form action.)
 */
...
function extensionPairingPostEvent(locals: Record<string, unknown>): RequestEvent {
  return {
    locals,
    request: new Request('http://x/api/settings/extension-pairing', { method: 'POST' }),
  } as unknown as RequestEvent;
}
...
describe('per-role permission enforcement (requireRole)', () => {
  ...
  describe('POST /api/settings/extension-pairing (admin-gated, no tenant guard)', () => {
    it('rejects a member with 403', async () => {
      const a = await seedOrgWithProject('re-pairing-member');
      const event = extensionPairingPostEvent({ org: { id: a.orgId, slug: 'x', role: 'member' } });
      await expect(extensionPairingPost(event)).rejects.toMatchObject({ status: 403 });
    });

    it('allows an admin', async () => {
      const a = await seedOrgWithProject('re-pairing-admin');
      const event = extensionPairingPostEvent({ org: { id: a.orgId, slug: 'x', role: 'admin' } });
      const res = await extensionPairingPost(event);
      expect(res.status).toBe(200);
    });

    it('allows an owner', async () => {
      const a = await seedOrgWithProject('re-pairing-owner');
      const event = extensionPairingPostEvent({ org: { id: a.orgId, slug: 'x', role: 'owner' } });
      const res = await extensionPairingPost(event);
      expect(res.status).toBe(200);
    });

    it('is a no-op when locals.org is unset (auth off / self-host)', async () => {
      const event = extensionPairingPostEvent({});
      const res = await extensionPairingPost(event);
      expect(res.status).toBe(200);
    });
  });
