...
4. **Draft at most one or two distinct posts for this run.** For each draft:
   ...
   - **Language** - if `campaign.config.voice.language` is set (`en` or `it`), write in that language. Otherwise, write in whichever language your own writing (`operator_voice`) is actually in - this is a proactive post with nothing to answer, so there is no post to match, and the dashboard's own interface language has nothing to do with what you are posting to Mastodon.
