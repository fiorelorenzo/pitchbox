...
- **A generated migration can be silently skipped, and `migrate` still says "migrations applied".** Drizzle applies a migration only when its journal `when` is greater than the newest `when` already recorded in the database, and `0013_assist_run_kind.sql` was hand-authored with `when: 1788960000000`, which is in the future. Every migration generated after it therefore carries a real timestamp that is _lower_ than the newest applied one, gets skipped, and leaves you with a schema that does not match `schema.ts` while the command reports success (measured 2026-09-07: `0014` and `0015` both no-opped exactly this way). After `migrate:generate`, check `shared/src/db/migrations/meta/_journal.json` and push the new entry's `when` past the highest value already in the file, then verify the tables really exist (`docker exec pitchbox-postgres psql -U pitchbox -d pitchbox -c '\dt'`) instead of trusting the output.
