...
**A project's description is written by an agent over its whole source set, and that is the second data flow.** `project_sources` is the set a person curates on the project page (a repository, a local folder, an upload, a website, a Mastodon account, a Hacker News author, a captured LinkedIn post or profile). `POST /api/projects/[id]/runs` takes **no body**: it starts one `project_extraction` run over every active source and refuses with `no_sources` when there are none, so nothing picks one source per run anymore. The agent resolves the set itself at tool-call time (`cli/src/commands/project.ts`): `project_extract_sources` hands over the text each non-tree source last fetched, flattened by `shared/src/project-source-content.ts`, while `project_extract_files` / `project_extract_read` walk a file tree with an optional `sourceId` (omitting it with several trees is an error naming the candidates, never a silent pick). There used to be a second, deterministic path that appended a bullet per source after a `<!-- pitchbox:sources -->` marker and asked the operator to accept it; it is gone, and it is the thing to not reintroduce - a source set exists so that an agent can read it.

Two traps in that flow. There is exactly **one** repository kind, `git`: its re-sync reads the GitHub API for a GitHub URL and proves any other host with `git ls-remote`, and a GitHub shorthand (`owner/repo`) is normalised to its https URL when the source is added, because `git clone` refuses the shorthand. And a description run over a repository needs the **`git` binary**, which `node:22-bookworm-slim` does not carry: `Dockerfile.app` installs it explicitly, so a cloud image that drops that line fails every repository run with ENOENT and nothing local reproduces it.
...
- **The docs site is a third styled surface, and it is not shadcn.** VitePress, `docs/.vitepress/theme/`: `tokens.css` is the verbatim token copy, `custom.css` maps those tokens onto VitePress's own `--vp-*` variables, and there are no component overrides (D39). Shoot it with `pnpm run docs:build && pnpm run docs:preview` (port 4173, no database needed) rather than `docs:dev`, since the built site is what ships; `uishot http://localhost:4173/ --theme light,dark` is the check. It is served from <https://docs.pitchbox.app> off GitHub Pages, so the VitePress `base` must stay the host root (D38). The domain is a repo Pages setting rather than anything in the tree: a workflow-built deploy does not read `docs/public/CNAME` (measured 2026-09-10 - it deployed with that file and `cname` stayed null), so it takes `gh api repos/fiorelorenzo/pitchbox/pages -X PUT -f cname=docs.pitchbox.app`, and `https_enforced` can only be set once the certificate reaches `approved`, a minute or two later.
...
**A project milestone is what used to be an epic issue.** Epics are not issues in Linear: a milestone costs no issue slot and shows progress natively. Keep a parent issue only for a deliverable that genuinely splits into sub-deliverables within a single agent run's reach. Every issue sits in a project and in a milestone, with no exceptions, and an issue filed in the middle of another issue's run is not exempt.
...
- Project and milestone: assign the project this work belongs to and, inside it, the milestone (the former epic) it serves. Both, always. A project with no milestone is the defect this repo actually produces: six issues filed on 2026-09-11 carried a project and no milestone, and the wording that used to sit here is what allowed it. If no milestone fits, create one in the project that owns the work rather than leaving the issue loose.
